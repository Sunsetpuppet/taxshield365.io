...
# --- CONFIGURATION ---
st.set_page_config(page_title="TaxShield FINAL v3.0", page_icon="🛡️", layout="centered")
...
# --- 2. PAYMENT LOGIC (With Fingerprint) ---
def create_stripe_session(ot_val, dt_val, tips_val):
    stripe.api_key = st.secrets["STRIPE_API_KEY"]
    # Create unique fingerprint of the current values
    fingerprint = f"{float(ot_val):.2f}|{float(dt_val):.2f}|{float(tips_val):.2f}"
...
def check_payment_status(session_id, current_ot, current_dt, current_tips):
    ...
    try:
        ...
        current_fingerprint = f"{float(current_ot):.2f}|{float(current_dt):.2f}|{float(current_tips):.2f}"
...
# --- 3. RESET HANDLER (This fixes the editing bug) ---
def reset_payment():
    # If the user edits a number, we wipe the old payment link
    # This forces the app to generate a NEW link that matches the NEW numbers.
...
def main():
    ...
    # Check for session returned from Stripe
    ...
    # --- UPLOAD SECTION ---
    # We disable upload ONLY if they have successfully paid and verified.
    # This aligns with "refresh to scan new".
    ...
    with st.container():
        with st.expander("📂 Upload Paystub", expanded=(st.session_state['report_data'] is None)):
            with st.form("scan_form", clear_on_submit=False):
                ...
                if submitted:
                    reset_payment() # Clear old data on new scan
    ...
    if st.session_state['report_data']:
        ...
        # --- THE FIX: 'on_change=reset_payment' ---
        # This tells Streamlit: "If they type here, kill the old payment button."
        with c1: ot_gross = st.number_input("Overtime YTD (Gross)", value=float(data.get('ytd_overtime_income', 0.0)), format="%.2f", disabled=is_locked, on_change=reset_payment)
        with c2: dt_gross = st.number_input("Double Time YTD (Gross)", value=float(data.get('ytd_double_time_income', 0.0)), format="%.2f", disabled=is_locked, on_change=reset_payment)
        with c3: tips = st.number_input("Tips YTD", value=float(data.get('ytd_tip_income', 0.0)), format="%.2f", disabled=is_locked, on_change=reset_payment)
                            ...
                            with st.spinner("Verifying Transaction..."):
                                ...
                                if not sid_to_check:
                                    ...
                                    if is_paid:
                                        ...
                                        st.error("It looks like you edited the numbers AFTER generating the payment link.")
                                        st.warning("Please simply click 'Unlock' again to generate a NEW link for your new numbers.")
                                        reset_payment() # Force them to make a new link
                                    else:
